...
public class Rope : MonoBehaviour
{
    ...
    // Объект твердого тела, к которому следует присоединить конец веревки.
    public Rigidbody2D connectedObject;
    // Максимальная длина звена веревки (если потребуется удлинить веревку больше, чем на эту величину, будет создано новое звено).
    ...
    void CreateRopeSegment()
    {
        ...
        else
        {
            ...
            // Установить начальную длину сочленения нового звена равной 0 - она увеличится автоматически.
            segmentJoint.distance = 0.0f;
        }
        // Соединить новое звено с опорой для веревки (то есть с объектом this)
        segmentJoint.connectedBody = this.GetComponent<Rigidbody2D>();
    }

    // Вызывается, когда нужно укоротить веревку, и удаляет звено сверху.
    ...
    // При необходимости в каждом кадре длина веревки удлиняется или укорачивается
    void Update()
    {
        ...
        if (isDecreasing)
        {
            // Веревку нужно удлинить. Если длина сочленения близка к нулю, удалить звено; иначе уменьшить длину сочленения верхнего звена.
        ...
        if (lineRenderer != null)
        {
            ...
            // Передать визуализатору координаты всех звеньев веревки.
            for (int i = 0; i < ropeSegments.Count; i++)
            {
                lineRenderer.SetPosition(i + 1, ropeSegments[i].transform.position);
            }
            // Последняя точка соответствует точке привязки несущего объекта.
            SpringJoint2D connectedObjectJoint = connectedObject.GetComponent<SpringJoint2D>();
