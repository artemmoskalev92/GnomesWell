...
public class Gnome : MonoBehaviour
{
    ...
    public void DestroyGnome(DamageType type)
    {
        ...
        foreach (BodyPart part in GetComponentsInChildren<BodyPart>())
        {
            ...
            if (shouldDetach)
            {
                // Обеспечить удаление твердого тела и коллайдера из этого объекта после достижения дна
                part.Detach();
                // Если часть тела отделена и повреждение имеет тип Slicing, добавить фонтан крови
